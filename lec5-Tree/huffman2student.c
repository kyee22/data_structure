...
void add2List(struct tnode *p){		//保证升序的插入
	...
	for(int i=total-1+headPos;i>=0+headPos;i--){//bug是total=0,即列表为空时,是不会进入循环的
		if(w<list[i]->weight){
			list[i+1]=list[i];
			list[i]=p;
		}
...
void visit(struct tnode *p,char code,int depth){
	if(depth>1)path[depth-2]=code;//前序

	if(p->left==NULL&&p->right==NULL){//递归边界：叶节点
		path[depth-1]='\0';
...
void atoHZIP()
{
	...
	unsigned char hch=0,*pBit=NULL;
	 ...
	 /*   由于Step 1已经读过Src, 所以需要重新调整offset   */
